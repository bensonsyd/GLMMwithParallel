...
neg.binomial.glmm <- function(){
  ...
  link <- "log"
  cum <- function(eta, nsuccesses){
    -nsuccesses*log(1-exp(eta))+nsuccesses*eta
  }
  cp <- function(eta, nsuccesses){
    (nsuccesses*exp(eta))/(1-exp(eta))+nsuccesses
  }
  cpp <- function(eta, nsuccesses){
    (nsuccesses*exp(eta))/((1-exp(eta))^2)
  }
  checkData<-function(x) {
    bads<-sum(x<0)
    if(bads>0) stop("data must be nonnegative integers.")
